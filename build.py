...
def generate_packer_vars(config, tools_yaml_path):
    ...
    # Normalize paths to forward slashes for HCL compatibility.
    # Packer accepts forward slashes on all platforms, but backslashes
    # in HCL strings are interpreted as escape sequences.
    def hcl_path(p):
...
output_directory = "{hcl_path(output.get("directory", "output"))}"
